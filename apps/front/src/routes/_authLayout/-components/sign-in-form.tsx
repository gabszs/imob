...
export function SignInForm() {
	...
	const socialSignInMutation = useMutation({
		mutationFn: async ({ provider }: { provider: "google" | "github" }) => {
			const callbackURL = `${import.meta.env.VITE_CLIENT_URL}/`;
	...
	return (
		<div className="mx-auto mt-10 w-full max-w-md p-6">
			...
			{!isOtpSent && (
				<>
					...
					<div className="relative my-6 flex items-center">
						<div className="flex-grow border-muted-foreground/20 border-t" />
			...
			<div className="mt-8 text-center text-muted-foreground text-sm italic">
				Secure authentication with Traki Analytics
			</div>
		</div>
	);
}
