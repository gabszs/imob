...
function ChatPage() {
	...
	const { selectedModelId, selectedModelIdRef, handleModelChange, settingsQuery } =
		useModelSelection();

	const { scrollRef, contentRef, isAtBottom, scheduleScrollToBottom } = useChatScroll();

	const { messages, status, error, sendMessage, title, shouldShowStreamingIndicator } =
		useChatOperations({
			chatId,
			selectedModelIdRef,
	...
	return (
		<div className="flex h-full flex-col overflow-hidden">
			<ChatHeader title={title} onDelete={handleDeleteConversation} isDeleting={isDeleting} />
			<ChatMessagesList
				messages={messages}
				error={error ?? null}
