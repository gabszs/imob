...
export function usePendingConversationMessage({
	...
	useEffect(() => {
		...
		const stored = window.sessionStorage.getItem(`better-chat:pending:${chatId}`);
	...
	useEffect(() => {
		if (!pendingText || status === "streaming" || messages.length > 0 || !isMessagesReady) {
			return;
		}
		...
		try {
			...
			if (typeof window !== "undefined") {
				try {
					window.sessionStorage.setItem(`better-chat:pending:${chatId}`, pendingText);
				} catch (_storageError) {
	...
	}, [chatId, isMessagesReady, messages.length, onError, pendingText, sendMessage, status]);
}
