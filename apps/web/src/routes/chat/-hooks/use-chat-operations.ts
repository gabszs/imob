...
interface UseChatOperationsParams {
	...
	scheduleScrollToBottom: (options?: { animation?: "instant" | "smooth"; force?: boolean }) => void;
...
export function useChatOperations({
	...
	const { messages, status, error, sendMessage, setMessages } = useChat<ChatMessage>({
		id: chatId,
	...
	const isMessagesReady =
		!messagesQuery.isLoading && !messagesQuery.isFetching && messagesQuery.isSuccess;
	...
	const title = (conversationQuery.data as ConversationSummary)?.title?.trim() || "New chat";
