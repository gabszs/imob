...
export function MessageRenderer({ message, proseClass = defaultProseClass }: MessageRendererProps) {
...
function UserPartRenderer({ part }: { part: MessagePart }) {
	if (isTextPart(part)) {
		return <p className="whitespace-pre-wrap text-sm md:text-base">{part.text}</p>;
	}

	if (isFilePart(part) && part.mediaType.startsWith("image/") && part.url) {
		return <img src={part.url} alt="User upload" className="max-w-full rounded" />;
	}
...
function ResponsePartRenderer({
	...
	if (isTextPart(part)) {
		return <MemoizedResponse content={part.text} id={messageId} proseClass={proseClass} />;
	}
