...
export function ChatSettingsDialog({ disabled, modelId, onModelChange }: ChatSettingsDialogProps) {
	...
	const effectiveValue = modelId ?? settingsQuery.data?.selectedModel ?? DEFAULT_MODEL_ID;
	const selectedModelDef = modelsWithAvailability.find(
		(model: ModelDefinition & { isAvailable: boolean }) => model.id === effectiveValue,
	);

	const hasCustomSelection = enabledModels.length > 0;
	const enabledModelSet = useMemo(() => new Set(enabledModels), [enabledModels]);
	const shouldIncludeModel = (model: ModelDefinition & { isAvailable: boolean }): boolean => {
	...
	const freeModels = visibleModels.filter(
		(model: ModelDefinition & { isAvailable: boolean }) => model.access === "free",
	);
	const byokModels = visibleModels.filter(
		(model: ModelDefinition & { isAvailable: boolean }) => model.access === "byok",
	);
	...
	const hasReasoningCapability = selectedModel?.capabilities?.includes("reasoning") ?? false;
	...
	const sortedServers = useMemo(() => {
		return [...(servers as Array<McpServer & { enabled: boolean }>)].sort((a, b) => {
	...
	return (
		<Dialog open={open} onOpenChange={setOpen}>
			<DialogTrigger asChild>
				<Button variant="outline" size="icon" disabled={disabled} title="Chat settings">
					...
					<div className="space-y-6">
										...
										<>
											{freeModels.length > 0 && (
												<>
													...
													{freeModels.map((model: ModelDefinition & { isAvailable: boolean }) => {
														const hasReasoning = model.capabilities?.includes("reasoning");
														return (
															<SelectItem key={model.id} value={model.id}>
																<div className="flex items-center gap-2">
																	<span>{model.name}</span>
																	{hasReasoning && <Brain className="size-3 text-purple-600" />}
																</div>
															</SelectItem>
														);
													})}
											...
											{byokModels.length > 0 && (
												<>
													...
													{byokModels.map((model: ModelDefinition & { isAvailable: boolean }) => {
														const hasReasoning = model.capabilities?.includes("reasoning");
														return (
															<SelectItem
																key={model.id}
						...
						{hasReasoningCapability && (
							<>
								<Separator />
								<div className="space-y-3">
									<Label className="font-semibold text-sm">Reasoning Effort</Label>
								...
								<div className="space-y-3">
									...
									<div className="flex items-center justify-between rounded-md bg-muted/60 px-3 py-2 text-muted-foreground text-xs">
										<div className="items-centered flex gap-2">
											...
											<span>Consider only enabling tools relevant to your immediate use case.</span>
										</div>
									</div>
								</div>
							)}
