...
export function ChatShell() {
	...
	useEffect(() => {
		...
		window.addEventListener("better-chat:open-history", handler as EventListener);
		return () => window.removeEventListener("better-chat:open-history", handler as EventListener);
	}, []);
	...
	const [localSelectedModelId, setLocalSelectedModelId] = useState<string | undefined>(
	...
	const selectedModelId = localSelectedModelId || settingsQuery.data?.selectedModel;
	...
	const getConversationList = (onSelect: (id: string) => void) => {
		...
		type Conversation = RouterOutputs["chat"]["listConversations"]["items"][number];
		...
		return (
			<div className="space-y-2">
				{conversations.map((conversation: Conversation) => {
					...
					const relativeUpdated = formatDistanceToNowStrict(new Date(conversation.updated), {
						addSuffix: true,
					});
					return (
						<Button
						...
						>
							...
							<span className="text-muted-foreground text-xs">{relativeUpdated}</span>
						</Button>
					);
				})}
			</div>
		...
		<div className="max-w-[100vw] overflow-x-hidden px-2 pt-13 sm:px-4">
			{isMobile && (
				<Sheet open={mobileSidebarOpen} onOpenChange={setMobileSidebarOpen}>
					<SheetContent side="left" className="w-[min(90vw,18rem)] border-r-0 p-0 sm:max-w-xs">
						<SheetHeader className="sr-only">
							<SheetTitle>Chat History</SheetTitle>
							<SheetDescription>Navigate between your conversations.</SheetDescription>
						</SheetHeader>
			...
			<div
				className={cn(
					...
					settingsQuery.data?.chatWidth === "comfortable" ? "max-w-7xl" : "max-w-5xl",
				)}
			>
				<aside className="relative hidden w-64 shrink-0 md:block">
					<div className="sticky top-13 flex h-[calc(100svh-3.25rem-1.5rem)] flex-col overflow-hidden border bg-card p-3 shadow-sm sm:p-4 md:h-[calc(100svh-3.25rem-0.5rem)]">
						...
						<Button onClick={startNewConversation} size="sm" className="mb-4 gap-2">
							<Plus className="size-4" /> New Chat
						</Button>
										...
										<div className="flex flex-col items-start space-y-3">
											{QUICK_PROMPTS.map((prompt) => (
												<Button
													...
													onClick={() => handleStartNewChat({ text: prompt.text })}
