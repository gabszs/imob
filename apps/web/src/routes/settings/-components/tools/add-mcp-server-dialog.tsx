		...
		<Dialog open={open} onOpenChange={onOpenChange}>
			<DialogContent>
				<DialogHeader>
					...
					<DialogDescription>Add a new MCP server to extend AI capabilities</DialogDescription>
				</DialogHeader>
				<div className="space-y-4">
					<div>
						...
						<Input
							...
							onChange={(e) => onServerChange({ ...newServer, name: e.target.value })}
					...
					<div>
						...
						<Input
							id={`${id}-url`}
							value={newServer.url}
							onChange={(e) => onServerChange({ ...newServer, url: e.target.value })}
					...
					<div>
						...
						<p className="mt-2 text-muted-foreground text-xs">
							Use HTTP for production deployments. SSE remains available for servers that only
					...
					<div className="flex justify-end gap-2">
						...
						<Button onClick={onAdd} disabled={isAdding || !newServer.name || !newServer.url}>
							{isAdding ? (
								<>
