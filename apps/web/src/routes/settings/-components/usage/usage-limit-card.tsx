...
export function UsageLimitCard({ usage }: UsageLimitCardProps) {
	const dailyPercentage = usage ? (usage.daily.used / usage.daily.limit) * 100 : 0;
	const monthlyPercentage = usage ? (usage.monthly.used / usage.monthly.limit) * 100 : 0;
			...
			<CardContent className="space-y-6">
				...
				{dailyPercentage >= 100 ? (
					<div className="border border-red-200 bg-red-50 p-3">
						<p className="font-medium text-red-900 text-sm">🚫 Daily limit reached</p>
						<p className="mt-1 text-red-800 text-xs">
							You've used all your free messages for today. Add your own API key in{" "}
							<a href="/settings/providers" className="underline hover:text-red-900">
								Provider Settings
							</a>{" "}
				...
				{monthlyPercentage >= 100 ? (
					<div className="border border-red-200 bg-red-50 p-3">
						<p className="font-medium text-red-900 text-sm">🚫 Monthly limit reached</p>
						<p className="mt-1 text-red-800 text-xs">
							You've used all your free messages for this month. Add your own API key in{" "}
							<a href="/settings/providers" className="underline hover:text-red-900">
								Provider Settings
							</a>{" "}
							for unlimited usage. Your free quota will reset on the 1st of next month.
						</p>
					</div>
