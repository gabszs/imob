		...
		>
			<div className="flex-1 space-y-2">
				...
				<div className="text-muted-foreground text-xs">
					...
					{model.maxOutputTokens && ` • Output: ${formatContext(model.maxOutputTokens)} •`}
					{model.costPer1MTokens && (
						<span className="ml-2 text-amber-600">Cost: {formatCost(model.costPer1MTokens)}</span>
					)}
				</div>

				{!available && (
					<p className="font-medium text-destructive text-xs">Requires API key to use</p>
				)}
			</div>
			...
			{isFreeModel ? (
				...
				<div className="flex items-center gap-2">
					...
					/>
					<span className="text-muted-foreground text-xs">{enabled ? "Shown" : "Hidden"}</span>
				</div>
			)}
		</div>
	);
