...
function SettingsLayout() {
	...
	useEffect(() => {
		...
		window.addEventListener("better-chat:open-settings", handler as EventListener);
		return () => window.removeEventListener("better-chat:open-settings", handler as EventListener);
	}, []);
			...
			>
				<aside className="relative hidden w-64 shrink-0 md:block">
					<div className="sticky top-13 flex h-[calc(100svh-3.25rem-1.5rem)] flex-col overflow-hidden border bg-card p-3 shadow-sm sm:p-4 md:h-[calc(100svh-3.25rem-0.5rem)]">
						...
						<div className="-mx-1 flex-1 overflow-y-auto px-1">{renderNavigation()}</div>
					</div>
				</aside>
