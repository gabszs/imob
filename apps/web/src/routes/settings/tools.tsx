...
function ToolsPage() {
	...
	const mcpServers = (serversQuery.data || []) as Array<MCPServer & { enabled: boolean }>;
		...
		<div className="space-y-6">
			<Card>
				<CardHeader>
					...
					<CardDescription>
						Enable real-time web search for current events and latest information.
					</CardDescription>
				</CardHeader>
				...
				<CardContent className="space-y-3">
					{mcpServers.map((server) => (
						<McpServerRow
							key={server.id}
							server={server}
							onToggle={(serverId, enabled) => toggleServerMutation.mutate({ serverId, enabled })}
					...
					{mcpServers.length > 0 && (
						<div className="flex items-center gap-2 rounded-md bg-muted/60 px-3 py-2 text-muted-foreground text-sm">
							...
							<span>
								Consider only enabling MCP servers relevant to your immediate use case to avoid
								context bloat.
							</span>
						</div>
					)}
			...
			{mcpServers.length === 0 && (
				<Card>
					<CardContent className="p-8 text-center">
						...
						<h3 className="mb-2 font-medium text-lg">No MCP Servers Configured</h3>
						<p className="mb-4 text-muted-foreground">
							Add your first MCP server to enable documentation search capabilities
						</p>
