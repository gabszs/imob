...
function UsageSettings() {
	...
	const modelsQuery = useQuery(orpc.models.list.queryOptions({ staleTime: 5 * 60 * 1000 }));
	...
	const getModelDisplayName = (modelId: string): string => {
		const model = modelsQuery.data?.find((m: ModelsList[number]) => m.id === modelId);
	...
	const modelEntries: Array<[string, ModelUsageEntry]> = stats?.totals?.models
		? (Object.entries(stats.totals.models) as Array<[string, ModelUsageEntry]>).sort(
