...
export function SignInForm({ redirectPath }: SignInFormProps) {
	...
	const socialSignInMutation = useMutation({
		mutationFn: async ({ provider }: { provider: keyof typeof SOCIAL_PROVIDERS }) => {
			const baseUrl =
		...
		onSuccess: (_, variables) => {
			toast.success(`Successfully signed in with ${SOCIAL_PROVIDERS[variables.provider]}`);
		},
		onError: (error: { error?: { message?: string } }, variables) => {
			toast.error(
				error.error?.message || `Failed to sign in with ${SOCIAL_PROVIDERS[variables.provider]}`,
			);
		},
	});
	...
	return (
		<div className="mx-auto mt-10 w-full max-w-md p-6">
			...
			<p className="mb-6 text-center text-muted-foreground">Choose a sign in method below.</p>
							...
							<emailForm.Subscribe>
								{(state) => (
									<Button
										...
										disabled={!state.canSubmit || state.isSubmitting || sendOtpMutation.isPending}
									>
									...
									<field.FormControl>
										...
										>
											<InputOTPGroup className="w-full justify-between gap-2">
												{Array.from({ length: SIGN_IN_FORM.OTP_LENGTH }, (_, index) => (
													<InputOTPSlot
						...
						<otpForm.Subscribe>
							{(state) => (
								<Button
									type="submit"
									className="w-full text-base"
									disabled={!state.canSubmit || state.isSubmitting || verifyOtpMutation.isPending}
								>
