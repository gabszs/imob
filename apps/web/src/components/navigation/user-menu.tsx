...
import { BookText, Github, Loader2, LogOutIcon, SettingsIcon, UserLock } from "lucide-react";
...
export default function UserMenu() {
	...
	const displayFirstName = user.data ? (getFirstName(user.data) ?? "User") : null;
			...
			<DropdownMenuContent className="w-56" align="end" forceMount>
				<DropdownMenuLabel className="font-normal">
					<div className="flex flex-col space-y-1">
						<p className="font-medium text-sm leading-none">{user.data?.name || "User"}</p>
						<p className="text-muted-foreground text-xs leading-none">{user.data?.email}</p>
					</div>
				</DropdownMenuLabel>
				<DropdownMenuSeparator />
