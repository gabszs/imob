...
import { type RouterInputs, type RouterOutputs } from "@/server/lib/router";
...
export function useUpdateUserSettings() {
	...
	const mutation = useMutation(
		orpc.settings.update.mutationOptions({
			onMutate: async (updates: RouterInputs["settings"]["update"]) => {
				...
				const previous = queryClient.getQueryData(orpc.settings.get.queryKey({}));
			...
			onSuccess: (updated: UserSettings) => {
				...
				if (updated.chatWidth === "cozy" || updated.chatWidth === "comfortable") {
			...
			onError: (_error, _variables, context) => {
				...
				if (context?.previous) {
					queryClient.setQueryData(orpc.settings.get.queryKey({}), context.previous);
				}
			},
		}),
	);
	...
	return useCallback(
		async (updates: Partial<UserSettings>) => {
			...
			return await mutation.mutateAsync(updates as RouterInputs["settings"]["update"]);
		},
		[mutation, userId],
	);
}
